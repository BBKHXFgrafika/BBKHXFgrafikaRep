...
void movement_of_objects(Move *move, Action *action, World *world) {
    ...
    if (action->call_satellite == TRUE && ((move->satellite[0].x) < 6000 || abs(move->satellite[1].x) < 6000)) {
        if (action->satellite_is_moving == 0) {
            move->satellite[0].x = -6000;
            move->satellite[0].y = 1500;
            move->satellite[0].z = 400;

            move->satellite[1].x = 5900;
            move->satellite[1].y = 1500;
            move->satellite[1].z = 600;

            move->satellite[2].x = 0;
            move->satellite[2].y = 6000;
            move->satellite[2].z = 500;
        }
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (inside_gravity_field) {
                Vertex distance_vector = vector_from_two_vertex(move->satellite[j].x, move->satellite[j].y,
                                                                ...
                                                                move->planets[i]->z);
                if (i != 4 && (j == 0 || j == 1)) { // every planet except Sun
                    move->satellite[j].x += 1.0 * sign;
                    ...
                    move->satellite[j].x += 4 * sign;
                    move->satellite[j].y += 0.3;
                }

                if (i != 4 && j == 2) { // every planet except Sun
                    move->satellite[j].y += 1.0 * sign;

                    if (distance_vector.x <= 0) {
                        move->satellite[j].x -= 1;
                    } else {
                        move->satellite[j].x += 1;
                    }
                } else { // Sun
                    move->satellite[j].x += 0.3;
                    move->satellite[j].y += 5 * sign;
                }
                inside_gravity_field = false;
            } else {
                switch (j) {
                    case 0:
                        move->satellite[j].x += 7 * sign;
                        break;
                    case 1:
                        move->satellite[j].x += 7 * sign;
                        break;
                    case 2:
                        move->satellite[j].y -= 7;
                        break;
                }
